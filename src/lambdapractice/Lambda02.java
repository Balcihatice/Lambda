...
public class Lambda02 {
    public static void main(String[] args) {
        ...
        System.out.println("ilkHarfBykGerisiKckList(list) = " + ilkHarfBuyukGerisiKucuk(list));
        System.out.println("ecIleBaslayanList(list) = " + ecIleBaslayanlar(list));
        yildizliYaz(list);
    ...
    public static List<String> ecIleBaslayanlar(List<String> list) {
        return
                list.stream().
    ...
    public static List<String> eOlanList (List<String> list) {
       return
        list.stream().filter(t -> t.toLowerCase().contains("e")).collect(Collectors.toList());
    }

     //S5: tum 'l' leri silelim yazdiralim
     public static  void tumlsil(List<String> list){
        list.stream().map(t->t.replace("l", "")).
...
public static void ikiHrfsiralaIlk5ElamEnBuyuk(List<String> list){
       list.stream().
               sorted(Comparator.comparing(t->t.charAt(1))).//list elemanlarini ikinci harfe gore sirala
               limit(5).//ilk 5 elean akisa devam etti
               sorted(Comparator.comparing(String::length).reversed()).//akistan gelen 5 elemani uzunlukarina gore ters siralama yapti
               findFirst();//akistan gelen ilk elemani aaldi.Optional String eleman dondurdu
}
